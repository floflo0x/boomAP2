...
router.post("/raffle", 
  ...
  async (req, res, next) => {
    ...
    try {
      const { title, des, image1, image2, image3, image4, image5, brand_img, retail_price, resell_price, cat_id, brand, date} = req.body;
...
router.get("/viewRaffle/:id", isAuth, async (req, res, next) => {
  try {
    ...
    const postData = { id: id };
    const response2 = await axios.request(
      insertUrl("get/raffleById.php", "post", postData)
    );
    const data2 = response2.data;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { title, image, link, des, date } = req.body;
      ...
      if (!error.isEmpty()) {
        ...
        const response = await axios.request(
          insertUrl("insert/ads.php", "post", { 'title': title, 'img': image, 'link': link, 'des': des, 'date': date })
        );
  ...
  async (req, res, next) => {
    try {
      ...
      if (!error.isEmpty()) {
        ...
        const response = await axios.request(
          insertUrl("update/ads.php", "post", { 'adId': id, 'title': title, 'img': image, 'link': link, 'des': des, 'date': date })
        );
